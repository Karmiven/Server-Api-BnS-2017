...
FROM DISK = 'D:\Server-Api-BnS-2017\DB\GameItemsDB.bak' -- Путь к файлу резервной копии (.bak). -- Path to the backup file (.bak).
WITH
...
FROM DISK = 'D:\Server-Api-BnS-2017\DB\DonationsDb.bak' -- Путь к файлу резервной копии DonationsDb (.bak). -- Path to the DonationsDb backup file (.bak).
WITH
    -- Задаём путь для основного файла данных (.mdf) после восстановления.
    -- Specifies the path for the main data file (.mdf) after restoration.

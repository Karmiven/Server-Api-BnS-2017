    ...
    try {
        ...
        try {
            const [creatureResult, deletedResult] = await Promise.all([
                        executeQuery(configBlGame01, 'SELECT COUNT(*) AS count FROM CreatureProperty'),
                        executeQuery(configBlGame01, 'SELECT COUNT(*) AS count FROM CreatureProperty WHERE deletion = 1')
                    ]);

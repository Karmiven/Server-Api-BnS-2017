...
async function loadItemsFromDB() {
    ...
    try {
        const itemsQuery = `
            SELECT [ItemID], [EN_Description] 
            FROM [GameItemsDB].dbo.[GameItems] 
            WHERE [EN_Description] IS NOT NULL AND [EN_Description] != ''
        `;
        
        const items = await executeItemTrackingQuery(configGameItemsDB, itemsQuery);
        
        itemsCache.clear();
        items.forEach(item => {
            itemsCache.set(item.ItemID, item.EN_Description);
        });
        
        itemsCacheTimestamp = now;
        log.info(`Loaded ${itemsCache.size} items from GameItemsDB`);
        
        return itemsCache;
    } catch (error) {
        log.error(`Error loading items from GameItemsDB: ${error.message}`);
        return new Map();
    }
}
...
router.get('/admin/item-tracking/mail', isAdmin, async (req, res) => {
    ...
    try {
        ...
        let query = `
            ...
            FROM [BlGame01].dbo.[UserPost] as a 
            INNER JOIN [BlGame01].dbo.[CreatureProperty] as b ON a.[SenderPCID] = b.[PCID]
            INNER JOIN [BlGame01].dbo.[CreatureProperty] as c ON a.[RecipientPCID] = c.[PCID]
            LEFT JOIN [BlGame01].dbo.[UserPostAttachment] as d ON d.[PostID] = a.[PostID]
        ...
        const mailData = await executeItemTrackingQuery(configBlGame01, query, params);
...
router.get('/admin/item-tracking/giftbox/data', isAdmin, async (req, res) => {
    ...
    try {
        ...
        let query = `
            ...
            LEFT JOIN [BlGame01].dbo.[CreatureProperty] as c ON a.[OwnerPCID] = c.[pcid] AND c.[deletion] = 0
            WHERE 1 = 1
        `;

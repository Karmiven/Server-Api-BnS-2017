...
// Функция для регистрации предмета в базе данных (адаптированная из WarehouseItemRoutes.js)
async function registerItem(ownerAccountID, goodsID, itemID, quantity, senderDescription, senderMessage) {
    ...
    try {
        ...
        // Сначала пробуем использовать процедуру
        try {
            ...
            log.debug(`Item registered successfully via procedure. LabelID: ${chalk.green(newLabelID)}`);
            ...
            // Если процедура не работает, используем fallback
            log.warning(`Procedure failed: ${procedureError.message}, using fallback method`);
            return await registerItemFallback(pool, ownerAccountID, goodsID, itemID, quantity, senderDescription, senderMessage, itemNumber);
        }
...
// Fallback метод для регистрации предметов
async function registerItemFallback(pool, ownerAccountID, goodsID, itemID, quantity, senderDescription, senderMessage, itemNumber) {
    try {
        // Вставляем запись в WarehouseGoods
        const insertGoodsQuery = `
            INSERT INTO WarehouseGoods (GoodsID, OwnerAccountID, GoodsNumber, SenderDescription, SenderMessage, PurchaseTime, RegistrationState)
            OUTPUT INSERTED.LabelID
            VALUES (@GoodsID, @OwnerAccountID, 233, @SenderDescription, @SenderMessage, @PurchaseTime, 2)
        `;
